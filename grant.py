...
class AuthRequestMixin(object):
    ...
    def read_validate_params(self, request):
        ...
        Implicit Grant.
        """
        if len(request.handler.request.body) > 1:
            body = json.loads(request.handler.request.body)
        else:
            body = {
        ...
        self.client = self.client_authenticator.by_identifier(body)

        if self.client.response_type_supported(body["response_type"]) is False:
...
class AuthorizationCodeTokenHandler(AccessTokenMixin, GrantHandler):
    ...
    def _read_params(self, request):
        ...
        self.client = self.client_authenticator.by_identifier_secret(body)
        self.code = body["code"]
        self.redirect_uri = body["redirect_uri"]
...
class AuthorizationCodeGrant(GrantHandlerFactory, ScopeGrant,
    ...
    def __call__(self, request, server):
        ...
        if (request.method == "POST" and request.path == server.token_path):
